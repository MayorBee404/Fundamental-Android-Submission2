...
class FollowerViewModel : ViewModel() {
    ...
    fun getUserFollowers(login: String): LiveData<Resource<List<DetailResource>>> {
        listUser.postValue(Resource.Loading())
        retrofit.getUserFollowers(login).enqueue(object : Callback<List<DetailResource>> {
            override fun onResponse(call: Call<List<DetailResource>>, response: Response<List<DetailResource>>) {
